...
**mhpfilter** implements the Modified Hodrick-Prescott (HP) Filter for
decomposing macroeconomic time series into trend and cyclical
components. The package automatically selects the optimal smoothing
parameter λ using generalized cross-validation (GCV), based on the
methodology of Choudhary, Hanif & Iqbal (2014).
...
- [`mhp_filter()`](https://myaseen208.com/mhpfilter/reference/mhp_filter.md) -
  Modified HP filter with automatic λ selection via GCV
- [`hp_filter()`](https://myaseen208.com/mhpfilter/reference/hp_filter.md) -
  Standard HP filter with fixed λ parameter
...
- [`print.mhp()`](https://myaseen208.com/mhpfilter/reference/print.mhp.md) -
  Print method for mhp class objects
